...
const session = conf.session.replace(/RAHMANI-XMD;;;=>/g, "");
require("dotenv").config({ path: "./config.env" });

let auto_reply_message = "Hello 👋,here an RAHMANI-XMD my owner is unavailable. Kindly leave a message.";
    ...
    zk.ev.on("call", async (callData) => {
      if (conf.ANTI_CALL === 'yes') {
        ...
        await zk.sendMessage(callerId, {
          text: "❗📵I AM RAHMANI-XMD| I REJECT THIS CALL BECAUSE MY OWNER IS NOT AVAILABLE FOR NOW. KINDLY SEND MESSAGE RIGHT NOW."
        });
      }
    });
    ...
    zk.ev.on("messages.upsert", async (m) => {
      ...
      if (ms.message.protocolMessage && ms.message.protocolMessage.type === 0) {
        ...
        if (deletedMessage) {
          ...
          let notification = `* RAHMANI-XMD ANTIDELETE💚*`;
